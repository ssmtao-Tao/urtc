...
2.外部采集有两种模式， 拉模式和推模式。  
    1.1 拉模式使用摄像头采集线程回调UCloudIVideoFrameObserver接口获取数据，此模式会占用摄像头
        设置顺序如下：
        enableExtendVideocapture();
        SetExtendMediaDataMode(UCloud_EMDM_PULL);
        registerVideoFrameObserver();
        
    1.2 推模式外部直接调用接口pushVideoFrameData pushAudioFrameData 推送数据给SDK， 推送的数据里需要包含时间戳信息。
        设置顺序如下：
        enableExtendVideocapture();
        ...
        pushVideoFrameData();
        pushAudioFrameData();
    1.3 模式的选择在发布UCLOUD_RTC_MEDIATYPE_VIDEO前设置，发布后不能切换。
